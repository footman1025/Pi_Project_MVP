...
const rows: Row[] = [
  ...
  { area: 'Opportunities catalog', kind: 'live', detail: 'Opportunity Hub: Live create (Supabase only) + twin-ranked catalog + Mark/Apply + owner notify; public /o/:slug SEO; Message with context; Featured = Soon; run supabase_opportunities_hub.sql + supabase_opportunity_interest_owner.sql' },
  { area: 'Opportunity Hub (market validation)', kind: 'live', detail: 'Loop: create → public page → apply → notify → message → outcomes metrics on /opportunities + /traction. Willingness-to-pay / featured = Soon.' },
