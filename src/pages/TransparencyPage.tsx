...
const rows: Row[] = [
  ...
  { area: 'Opportunities catalog', kind: 'live', detail: 'Opportunity Hub: Live create + twin-ranked catalog + Mark/Apply + owner notify; public /o/:slug SEO; Message with context; Featured listing (€9/7d Stripe or intent fallback); run supabase_opportunities_hub.sql + supabase_opportunity_featured.sql' },
  { area: 'Opportunity Hub (market validation)', kind: 'live', detail: 'Loop: create → public page → apply → notify → message → outcomes. Featured willingness-to-pay live (Stripe Checkout when STRIPE_SECRET_KEY set; else intent recorded).' },
  { area: 'Featured opportunity listings', kind: 'live', detail: 'Owner Feature CTA → Stripe Checkout (€9 / 7 days) or honest intent if Stripe unset; pins to top + badge; confirm via /api/featured-confirm' },
  ...
  { area: 'Creators / Professionals', kind: 'partial', detail: 'Live member discovery + Message/Profile; tip intent UI (Partial/Demo, no Stripe); courses & booking = Soon' },
  { area: 'Creator tip intents', kind: 'partial', detail: '/creators + public profiles — amount chips + optional note; records tip intent only (local or creator_tip_intents SQL); no card charged; Stripe checkout = Soon' },
  ...
  { area: 'Stripe / courses / live payouts', kind: 'soon', detail: 'Real payments, courses, and tip payouts planned after Investor Readiness' },
